...
당신이 잃하는 시간에 편의점에 채워놓을 물건이 들어오며
...
2. 만약 표의 값과 갯수가 일치하지 않는다면
   "전산표와 일치하지 않습니다"를 로그합니다.

  같아면 "전사표와 일치합니다"를 로그 합니다.
...
function 편의점(name, amount) {
  const itmesAmount = items[name];
  //console.log(para);
  const itemskey = Object.keys(items);
  //console.log(itemskey);
  let index = itemskey.findIndex((item) => item === name);
  if (index === -1) {
    console.log(`${name}은 존재하지 않는 상품입니다`);
  } else if (amount === itmesAmount) {
